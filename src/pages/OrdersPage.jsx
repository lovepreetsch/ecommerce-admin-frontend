...
export default function OrdersPage() {
  ...
  useEffect(() => {
    dispatch(fetchAllOrders({ page: 0, size: 50 }));
  }, [dispatch]);
  ...
  const confirmDelete = () => {
    // Backend doesn't support delete yet, usually we just cancel
    toast.info('Order deletion is not supported. Please cancel the order instead.');
      ...
      return (
        <div className="space-y-6">
          <div className="flex flex-col items-center text-center">
            ...
            <p className="text-sm text-textMuted mt-2">Are you sure you want to delete order "{selectedOrder?.orderNumber}"? This action cannot be undone.</p>
          </div>
      ...
      return (
        <div className="space-y-6">
          <div className="flex justify-between items-center bg-surfaceHover/50 p-4 rounded-xl border border-surfaceHover">
            <div>
              <p className="text-sm text-textMuted">Order Number</p>
              <p className="text-lg font-bold text-white">{selectedOrder.orderNumber}</p>
            </div>
            <div className="text-right">
              <p className="text-sm text-textMuted">Order Date</p>
              <p className="text-sm font-medium text-white">{selectedOrder.createdAt}</p>
            </div>
          </div>
          <div className="grid grid-cols-2 gap-6">
            <div className="col-span-2">
              <p className="text-sm text-textMuted mb-2">Order Items</p>
              <div className="bg-background rounded-lg border border-surfaceHover overflow-hidden">
                <table className="w-full text-left text-sm">
                  <thead className="bg-surface/50 text-textMuted">
                    <tr>
                  ...
                  <tbody>
                    {selectedOrder.items?.map(item => (
                      <tr key={item.id} className="border-t border-surfaceHover">
                        <td className="p-3 text-white">{item.productName}</td>
                        <td className="p-3 text-textMuted">{item.quantity}</td>
            ...
            <div className="col-span-2">
              ...
              <div className="bg-background p-4 rounded-lg border border-surfaceHover flex justify-between items-center">
                <span className="text-sm text-textMuted">Current Status</span>
          ...
          <div className="pt-4 border-t border-surfaceHover flex justify-between items-center">
             <p className="text-lg font-bold text-white">Total Amount: ${parseFloat(selectedOrder.totalAmount).toFixed(2)}</p>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {loading && orders.length === 0 ? (
                <tr><td colSpan="5" className="py-12 text-center text-textMuted">Loading orders...</td></tr>
              ) : filteredOrders.map((order) => (
                <tr key={order.id} className="border-b border-surfaceHover/50 hover:bg-surfaceHover/30 transition-colors group">
                  <td className="py-4 px-6"><span className="text-sm font-medium text-white">{order.orderNumber}</span></td>
                  <td className="py-4 px-6 text-sm text-textMuted">User #{order.userId}</td>
                  <td className="py-4 px-6"><span className={getStatusBadge(order.status)}>{order.status}</span></td>
                  <td className="py-4 px-6 text-sm font-medium text-white">${parseFloat(order.totalAmount).toFixed(2)}</td>
              ...
              {!loading && filteredOrders.length === 0 && <tr><td colSpan="5" className="py-12 text-center text-textMuted">No orders found.</td></tr>}
            </tbody>
          </table>
        </div>
      </div>
